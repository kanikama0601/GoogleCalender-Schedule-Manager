...
### 3. サーバーでの起動
実際のサーバー（Ubuntu 等の VPS）に SSH 接続し、以下の手順を実行します。

1. リポジトリをクローンします:
   ```bash
   git clone https://github.com/あなたのユーザー名/GoogleCalender-Schedule-Manager.git
   cd GoogleCalender-Schedule-Manager
   ```
2. 認証情報を配置します:
   手元のPCで取得した `credentials.json` を、サーバー上の `server/data/credentials.json` に安全に転送・配置します。
3. **本番用** の Compose ファイルを指定してバックグラウンド起動します:
   ```bash
   docker-compose -f docker-compose.prod.yml up -d
   ```
4. ブラウザから `http://あなたのサーバーのIPアドレス` (※フロントエンドはポート80番です) にアクセスすれば、利用可能になります！
